...
| スクリプト | 実行範囲 | 説明 |
|--------|----------------|-------------|
| **`run_agent.sh`**（引数なし） | **インタラクティブTUI** | **インタラクティブダッシュボード — 設定、タスク実行、履歴閲覧** |
| **`run_agent.sh "タスク"`** | **ステージ1 → 2 → 3** | **CLIモード — 自然言語入力 → フルパイプライン** |
...
> `run_agent.sh` 引数なしで**インタラクティブTUIダッシュボード**を起動します（TTY必要）。
> `run_agent.sh "自然言語タスク"`は**CLIモード**でパイプラインを実行します。
> `--mode`オプションで個別ステージを実行することもできます。
