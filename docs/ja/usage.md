# 使用方法
🇺🇸 [English](../usage.md) | 🇰🇷 [한국어](../ko/usage.md) | 🇨🇳 [中文](../zh/usage.md) | 🇯🇵 [日本語](usage.md) | 🇩🇪 [Deutsch](../de/usage.md)
...
## run_agent.sh -- フルパイプライン実行（メイン機能）

自然言語のタスク記述を1つ入力するだけで、NL→YAML→IsaacLab→DataCollectionのパイプライン全体が自動的に実行されます。
