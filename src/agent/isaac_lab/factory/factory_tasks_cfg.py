...
class PegInsert(FactoryTask):
    ...
    asset_size = 8.0
    duration_s = 50.0  # 5x longer for demo recording

    # Robot — start higher and with wider XY range for longer demos
    hand_init_pos: list = [0.0, 0.0, 0.15]  # 150mm above (3x original 47mm)
    hand_init_pos_noise: list = [0.06, 0.06, 0.03]  # 3x original noise
    hand_init_orn: list = [3.1416, 0.0, 0.0]
    hand_init_orn_noise: list = [0.0, 0.0, 0.785]

    # Fixed Asset — NO position randomization (hole stays in place)
...
class GearMesh(FactoryTask):
    ...
    # Robot — higher start + wider XY noise
    hand_init_pos: list = [0.0, 0.0, 0.105]  # 3x original 35mm
    hand_init_pos_noise: list = [0.06, 0.06, 0.03]  # 3x original
    hand_init_orn: list = [3.1416, 0, 0.0]
    hand_init_orn_noise: list = [0.0, 0.0, 0.785]

    # Fixed Asset — NO position randomization (gear base stays in place)
    fixed_asset_init_pos_noise: list = [0.0, 0.0, 0.0]
    fixed_asset_init_orn_deg: float = 0.0
    fixed_asset_init_orn_range_deg: float = 0.0
...
class NutThread(FactoryTask):
    ...
    asset_size = 16.0
    duration_s = 150.0  # 5x longer

    # Robot — higher start + wider XY noise
    hand_init_pos: list = [0.0, 0.0, 0.045]  # 3x original 15mm
    hand_init_pos_noise: list = [0.06, 0.06, 0.03]  # 3x original
    ...
    # Fixed Asset — NO position randomization (bolt stays in place)
    fixed_asset_init_pos_noise: list = [0.0, 0.0, 0.0]
    fixed_asset_init_orn_deg: float = 0.0
    fixed_asset_init_orn_range_deg: float = 0.0
