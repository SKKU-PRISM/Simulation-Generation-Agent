...
class TokenTracker:
    ...
    def report(self) -> str:
        ...
        lines = [
            "=== Token Usage Report ===",
            f"Total: {s['total_tokens']:,} tokens ({s['total_input_tokens']:,} in + {s['total_output_tokens']:,} out)",
        ...
        for step, data in sorted(s["by_step"].items()):
            ...
            lines.append(f"  {step}: {total:,} tokens ({data['calls']} calls)")
        lines.append("")
        lines.append("By Model:")
        for model, data in sorted(s["by_model"].items()):
            total = data["input_tokens"] + data["output_tokens"]
            lines.append(f"  {model}: {total:,} tokens ({data['calls']} calls)")
        return "\n".join(lines)
